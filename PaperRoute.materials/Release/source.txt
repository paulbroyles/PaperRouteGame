...
The release number is 2. [Changes since version 1 include fixing an error that made it impossible to end the game by exiting the office building, clarifying a few comments, and adding an instruction that strips articles from commands, and marking several items as nonportable.]
...
A window is a kind of thing. A window is always scenery. A window is always fixed in place. A window can be open or closed. A window is usually closed. A window can be openable or unopenable. A window is usually openable. [This definition is borrowed from §11.17 of the Inform documentation. The authors of the documentation invite you to borrow examples without credit, but for our class, you should note any borrowings in comments; a citation like the previous sentence is fine.]
...
Printing is an action applying to one visible thing. Understand "print [something]" as printing.
After deciding the scope of the player while printing, place the paper in scope. [This is a fairly complicated instruction. Normally when the player asks to print the paper, the game will complain, because the player is trying to do something to the paper, but the paper isn't in the room. Adjusting the scope of the paper is a way around that, but we have to be careful, or the player will be able to do other things to the paper as well.]
Instead of printing something when the computer is not visible, say "Your mother may accuse you of living online, but you don't really, so you can't print anything without having your computer in front of you."
Instead of printing something when the computer is switched off, say "You know, they call them electronic devices because they need electricity. Without that, they're just big boxes of... Oh, anyway, there's no way to print with the computer off."
Instead of printing something that is not the paper, say "The only thing you have due today is that paper, so there's no point printing anything else!"
Instead of printing the paper when the paper is not found for the first time, say "You'll have to find your paper before you can print it."
Instead of printing the paper when the paper is not found, say "You still haven't found the paper. Have you tried searching for it?".
Instead of printing the paper when the paper is queued, say "You've already printed it once; doing it again won't help.[if the paper is carried] And you already have it! Let's go![else if the paper is queued] Now you just have to figure out where it went![else if the paper is printed] Just take it with you and go.[end if]".
...
Does the player mean printing the paper: it is very likely.
